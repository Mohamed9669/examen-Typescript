...
import { addApprovisionnement, getApprovisionnements, updateApprovisionnement } from '../service/ApprovisionnementService';
import { render as renderListeAppro } from './ListeAppro';
...
export function render(editData?: Approvisionnement) {
  const app = document.getElementById('app');
  if (!app) return;
  app.innerHTML = `
    <h1 class="text-2xl font-bold mb-4">${editData ? 'Modifier' : 'Nouvel'} approvisionnement</h1>
    <form id="form-appro" class="space-y-4">
      ...
      <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded">${editData ? 'Mettre à jour' : 'Enregistrer'}</button>
    </form>
    <style>
  ...
  let lignes: LigneApprovisionnement[] = editData ? [...editData.lignes] : [];
  let refCourante = '';

  if (editData) {
    const refInput = document.getElementById('ref') as HTMLInputElement;
    if (refInput) refInput.value = editData.reference;
  } else {
  ...
  getFournisseurs().then(data => {
    ...
    if (editData) select.value = String(editData.fournisseurId);
  });
  ...
  function renderLignes() {
    ...
    lignes.forEach((ligne, idx) => {
      ...
      const selectOptions = articles.map(a => `<option value="${a.id}" ${a.id === ligne.articleId ? 'selected' : ''}>${a.nom}</option>`).join('');
      const prix = getPrixArticle(ligne.articleId);
      const html = `
        <div class="flex gap-2 mb-2 items-center" id="${ligneId}">
          ...
          <input type="number" min="1" value="${ligne.quantite}" class="quantite-input border p-1 w-20" data-idx="${idx}" />
  ...
  document.getElementById('form-appro')?.addEventListener('submit', async (e) => {
    ...
    if (editData) {
      await updateApprovisionnement(editData.id, { reference: ref, fournisseurId, date, lignes: lignesValides });
      showToast('Approvisionnement modifié !');
      setTimeout(() => renderListeAppro(), 800);
    } else {
